...
var abstractHomePageTests = function (state_name, action_name, loginServ) {
    ...
    spyOn(loginServ, "getUser").and.returnValue({
        "actions": [{"actions__name": action_name}]
    });
    spyOn(loginServ, "isLoggedIn").and.returnValue(true);
    inject(["emr.auth.services.homePage",
        function (hpg) {
...
describe("Unit Test: emr.auth.homePage",function () {
    var loginService;
    beforeEach(function () {
        module("emr.auth.services");
        module("emrApp.constants");
        inject(["emr.auth.services.login", function (loginServ) {
            loginService = loginServ;
        }]);
    });
    it("should change state to `scheduling` if a user has `view_schedule` acts",
        function () {
            abstractHomePageTests("scheduling", "view_schedule", loginService);
        });
