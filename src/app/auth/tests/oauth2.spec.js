(function (angular, jQuery, moment) {
    ...
    describe("Test emr.auth.oauth2 service :", function () {
        ...
        it("should fetch a token from oauth2 provider and store it",
            function () {
                inject(["$window", "$httpBackend", "CREDZ", "api.oauth2",
                    function ($window, $httpBackend, credz, oauth2) {
                        ...
                        var token = JSON
                            .parse($window.localStorage.getItem(store_key));
                        expect(token.expires_in)
                            .toEqual(access_token.expires_in);
                        expect(token.scope).toEqual(access_token.scope);
                        expect(token.access_token)
                            .toEqual(access_token.access_token);
                        expect(token.refresh_token)
                            .toEqual(access_token.refresh_token);
                        expect(moment(token.expire_at))
                            .toBeGreaterThan(moment());
                    }
                ]);
            }
        );
        ...
        it("should not issue multiple unresolved requests to fetch a token",
            function () {
        ...
        it("should refresh a token from oauth2 provider and store new token",
            function () {
                inject(["$window", "$httpBackend", "CREDZ", "api.oauth2",
                    function ($window, $httpBackend, credz, oauth2) {
                        $window.localStorage.setItem(store_key,
                        ...
                        var token = JSON.parse($window
                            .localStorage.getItem(store_key));
                        expect(token.expires_in)
                            .toEqual(refresh_token.expires_in);
                        expect(token.scope)
                            .toEqual(refresh_token.scope);
                        expect(token.access_token)
        ...
        it("should set XHR authorization headers after fetching a token",
            function () {
                inject(["$window", "$httpBackend", "$http", "CREDZ",
                    "api.oauth2",
                    function ($window, $httpBackend, $http, credz, oauth2) {
                        ...
                        var token = JSON.parse($window.localStorage
                            .getItem(store_key));
                        var header_value = token.token_type + " " +
                            token.access_token;

                        expect($http.defaults.headers.common.Authorization)
                            .toEqual(header_value);
                        expect(jQuery.ajaxSettings.headers.Authorization)
                            .toEqual(header_value);
                    }
                ]);
            }
        ...
        it("should set a token refresh timeout", function () {
            inject(["$window", "$httpBackend", "$timeout", "CREDZ",
                "api.oauth2",
        ...
        it("should not set XHR authorization headers it token is not defined",
            function () {
